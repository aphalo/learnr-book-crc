...
\begin{table}[h]
  \centering
    \caption[Functions related to theoretical probability distributions]{Theoretical probability distributions in \Rlang. Partial list of base \Rlang functions related to probability distributions. The full list can be obtained by executing the command \code{help(Distributions)}.\vspace{1ex}}\label{tab:prob:funs}
