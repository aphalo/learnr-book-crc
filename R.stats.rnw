...
In some situations, e.g., calculation of correlations, models lacking a lhs term (a term on the left hand side of \verb|~|) are used. At least one term must be present in the rhs of model formulas, as an expression ending in \code{~} is syntactically incomplete.
