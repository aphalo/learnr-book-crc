...
\begin{explainbox}
   We will show later in this chapter additional useful examples of the use of positions. Here we demonstrate the displacements they introduce using \gggeom{geom\_point()} and \gggeom{geom\_point\_s()}. \gggeom{geom\_point\_s()}  from package \pkgname{ggpp} makes the displacement visible by drawing an arrow connecting original and displace positions for each observation. \ggposition{position\_identity()} is the default for \gggeom{geom\_point()}. Position functions are passed as argument to formal parameter \code{position} of \emph{geometries}.

<<scatter-13a, eval=eval_plots_all>>=
ggplot(data = mtcars, aes(x = factor(cyl), y = mpg)) +
  geom_point() +
  geom_point_s(position = position_jitter(width = 0.05), 
               colour = "segment",
               colour.target = "all")
@
