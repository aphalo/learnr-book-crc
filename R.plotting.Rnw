...
The use of \code{expression} is explained in detail in section \ref{sec:??}, an the use of \code{scales} in section \ref{sec:???}.

\end{explainbox}

\section{Plotting text and expressions}\label{sec:plot:text}
...
\begin{playground}
Justification values outside the range $0\ldots 1$ are allowed, but are relative to the width of the label. As the default font used in this case has variable width characters
justification of text would be inconsistent if we displace them by adjusting \code{hjust}. Edit the code above using \code{hjust} set to 3 instead of to 1, without pasting a space character to the labels. Is the plot obtained ``tidy'' enough for publication? and for data exploration?
\end{playground}
...
\begin{playground}
See R's `plotmath' demo for more information on the syntax of expressions and play with the code above by altering the character strings in variable \code{label} of data frame \code{my.data}. How would you plot superscripts? and convert the numbers 1 to 5 in the labels into letters starting from ``i'' and use them as indexes? (Hint: see help for \code{letters}, a predefined vector available in R.)
\end{playground}

\begin{explainbox}
  In the examples above we plot text and expressions present in the data frame passed as argument for \code{data}. It is also possible to build suitable labels on-the-fly within \code{aes} when setting the mapping for \code{label}.
...
\subsection{Pie charts vs.\ bar plots example}
