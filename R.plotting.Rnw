...
\index{plots!scatter plot|(}The first examples of the use of \gggeom{geom\_point()} are for \textbf{scatter plots}, as \code{disp} and \code{mpg} are \code{numeric} variables. In the examples above a third variable, \code{cyl} was represented by colours. While the color aesthetic can be used with all \code{geoms}, other aesthetics can be used only with same \code{geoms}, for example the \code{shape} aesthetic can be used only with \gggeom{geom\_point()} and \code{geoms} like \gggeom{geom\_pointrange()} and \gggeom{geom\_point\_s()}. The values in the shape aesthetic are discrete, and consequently only discrete values can be mapped to it.
