...
p.inset <- p.main +
  ...
  labs(x = NULL, y = NULL, subtitle = "Detail of highlighted region") +
...
<<plot-npc-eb-01, eval=FALSE>>=
ggplot(data = mtcars, aes(x = disp, y = mpg,
                          colour = factor(cyl))) +
  geom_point() +
  annotate(geom = "text_npc",
           npcx = 0.5, npcy = 0.9, label = "a label")
@
\end{explainbox}
...
In addition to discrete scales for \emph{aesthetics} that are inherently discrete, such as the \code{shape} of symbols and \code{linetype}, discrete scales are available in addition to continuous scales for those \emph{aesthetics} that are inherently continuous, like \code{x}, \code{y}, \code{size}, \code{colour}, etc.\

Default scales set the mapping automatically (e.g.\ which colour corresponds to $x = 0$ and which one to $x = 1$). Scales allow users to tweak how the automatic mapping is done (e.g.\ passing as argument a colour palette different from the default one). An finally in the case of discrete scales the mapping between each value in data and a value for the \emph{aesthetic} can be set manually.

\begin{warningbox}
Aesthetics in a plot in addition to being determined by mappings, can be also set to a constant value (e.g.\ when we want all symbols in a plot to be red instead of the default of black). Aesthetics set to constant values, are not mapped to data, and consequently independent of scales. This means that many properties of, for example, the symbols used in a plot can be either set by a constant, or mapped to data.
\end{warningbox}

The most direct mapping to data is \code{identity}, which means that the data is taken at its face value. In a colour scale, say \ggscale{scale\_colour\_discrete()}, the variable in the data would be encoded with values such as \code{"red"}, \code{"blue"}---i.e. valid \Rlang colours. In a simple mapping levels of a factor, such as \code{"treatment"} and \code{"control"} would be represented as distinct colours with the correspondence of individual levels to individual colours determined by \code{scale\_colour\_discrete()}.

A continuous data variable needs to be mapped to an \emph{aesthetic} through a continuous scale. If we have a \code{numeric} variable that we want to map to colour, we have to use \code{scale\_colour\_continuous()} (e.g.\ with scale settings so that the colour varies gradually between \code{"red"} and \code{"grey50"} for the range of values present in \code{data}.
...
<<scale-limits-02, eval=eval_plots_all>>=
ggplot(fake2.data, aes(z, y)) + geom_point() +
...
<<scale-limits-06, eval=eval_plots_all>>=
ggplot(fake2.data,
  aes(fill = group, colour = group, x = y)) +
  stat_density(alpha = 0.3) +
...
<<scale-trans-02, eval=eval_plots_all>>=
ggplot(fake2.data, aes(z, y)) +
  geom_point() +
...
<<scale-discrete-10a, echo=3:4, eval=eval_plots_all>>=
ggplot(mpg, aes(class, hwy)) +
  stat_summary(geom = "col", fun.y = mean, na.rm = TRUE) +
  scale_x_discrete(limits = c("compact", "subcompact", "midsize"),
                   labels = toupper)
