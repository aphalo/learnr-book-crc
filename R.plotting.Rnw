...
As \code{geom\_label} obeys the same parameters as \code{geom\_text} except for \code{angle} we describe below only the additional parameters compared to \code{geom\_text}.
