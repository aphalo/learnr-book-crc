...
In the next example we plot the groups in separate panels, and map \code{level}, a variable computed by \code{stat\_density\_2d()}, to the \code{fill} \emph{aesthetic} and we map to it the variable \code{level}.
...
p.base <-
   ggplot(data = mtcars[1:8, ],
    ...
    geom_point()
p1 <- p.base + geom_line()
p2 <- p.base + geom_line(orientation = "y", colour = "red")
@
...
<<flipping_box-01b-ggplot>>=
p1 + p2
@

%<<flipping_box-01b-ggplot>>=
%p1 + geom_line(alpha = 0.5)
%@
%
%<<flipping_box-01c-ggplot>>=
%p1 + geom_line(alpha = 0.5, orientation = "y", colour = "red")
%@
%
The next pair of examples exemplifies automatic flipping using \ggstat{stat\_boxplot()}. Here we map the factor \code{Species} first to $x$ and then to $y$. In both cases boxplots have been computed and plotted for each level of the factor. Statistics \ggstat{stat\_boxplot()}, \ggstat{stat\_summary()}, \ggstat{stat\_histogram()} and \ggstat{stat\_density()} behave similarly with respect to flipping.
...
Using facet wrap the same plot would have been coded as follows.

<<facets-01a, eval=eval_plots_all>>=
p + facet_wrap(facets = vars(cyl), nrow = 1)
@
...
<<facets-02a>>=
p + facet_wrap(facets = vars(cyl), nrow = 1, scales = "free_y")
@

<<facets-02b, eval = FALSE, include = FALSE>>=
p + facet_grid(cols = vars(cyl), scales = "free_y", space = "free_y")
@
...
<<facets-05>>=
p + facet_grid(rows = vars(vs), cols = vars(am), labeller = label_both)
@
