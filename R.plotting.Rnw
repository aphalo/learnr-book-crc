...
\begin{warningbox}
If you are using version 2.1.0 or earlier of \ggplot, you will need to create the plots using \gggeom{geom\_bar()} instead of \gggeom{geom\_col()}. The code for the first figure in this section would need to be edited as bellow---this code also gives the desired plot with later versions of \ggplot.
...
Frequently, simpler program code can be written when null objects and null operations are available. If one adds \code{NULL} to a \code{ggplot} object, the same object is returned unchanged. In contrast, currently  (\pkgname{ggplot2} = 2.2.1), if an ``empty'' list is added, a copy of the object is returned. In the two cases the rendered plot is the same.
