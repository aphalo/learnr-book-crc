...
<<facets-13>>=
p + facet_wrap(facets = vars(cyl), ncol = 2)
@
...
p + facet_wrap(facets = vars(vs, am), ncol=2, labeller = label_both)
@
...
<<axis-labels-01, eval=eval_plots_all>>=
ggplot(data = Orange,
  ...
  ggtitle(title = "Growth of orange trees",
...
<<axis-labels-03, eval=eval_plots_all, echo=6:11>>=
ggplot(data = Orange,
       aes(x = age, y = circumference, color = Tree)) +
  geom_line() +
  geom_point() +
...
<<scale-ticks-03, eval=eval_plots_all>>=
...
<<inset-01>>=
p +
  annotation_custom(ggplotGrob(p + coord_cartesian(xlim = c(12.5, 17.5),
                                                   ylim = c(3.17, 4.17))),
                    xmin = 25.5, xmax = 34.5, ymin = 2.5, ymax = 5.5)
@
