...
\begin{table}
  \caption[Positions]{Position functions}\vspace{1ex}\label{tab:plots:position}
  \centering
  \noindent
...
Parameter \code{breaks}\index{plots!scales!tick breaks} is used not only to set the location of ticks along the axis in scales for the \code{x} and \code{y} aesthetics, but also for the keys or guides for other continuous scales such as those for color. Parameter \code{labels}\index{plots!scales!tick labels} is used to set the break labels, including tick labels. The argument passed to each of these parameters can be vector or a function. The default is to compute ``good'' breaks based on the limits and use to nice numbers suitable for labels.
...
With date values mapped to $x$ or $y$, tick labels are created with functions \Rfunction{label\_date()} or \Rfunction{label\_date\_short()}. In the case of time, tick labels are created with function \Rfunction{label\_time()} (see section \ref{sec:plot:scales:time:date} on page \pageref{sec:plot:scales:time:date}).
...
Time scales are similar to continuous scales for \code{numeric} values. In \Rlang and many other computing languages, time values are stored as integer or numeric values subject to special interpretation. Times stored as objects of class  can be mapped to continuous \emph{aesthetics} such as \code{x}, \code{y}, \code{color}, etc. Special scales for different aesthetics are available for time-related data.

Limits and breaks are preferably set using constant values of class \code{POSIXct}. These are most easily input with the functions in packages \pkgname{lubridate} or \pkgname{anytime} that decode dates and times available as character strings.

<<echo=FALSE, include=FALSE>>=
opts_chunk$set(opts_fig_very_wide)
@
\begin{explainbox}
In the next two chunks scale limits to subset a part of the observations present in \code{data}. Passing \code{na.rm = TRUE} when calling the \code{geom} functions silences the warning messages.
\end{explainbox}
