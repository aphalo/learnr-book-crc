...
\textbf{Status as of 2017-07-27.} Small edits to chapters 1 and 2. Expanded section on \pkgname{ggpmisc} to include the new functionality added in version 0.2.15.9002. Temporarily disabled evaluation of \pkgname{ggtern} and \pkgname{ggnetwork} examples, as \pkgname{ggplo2} $>=$ 2.2.1.9000 breaks them. This newer version of \pkgname{ggplo2} is required by the current development version of \pkgname{ggpmisc}.
