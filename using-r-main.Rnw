...
\textbf{Status as of 2017-03-14.} Revised and expanded the chapter on plotting maps, adding a section on the manipulation and plotting of image data. Revised and expanded the chapter on extensions to ggplot2, so that there are no longer empty sections.
\end{infobox}
...
<<child-r-friends, child='R.friends.Rnw', eval=incl_all || TRUE>>=
@
