...
We use for examples data frames from package \pkgname{datasets}, included in the \Rpgrm distribution.
...
The statement above returns a vector of character strings, with the mode of each column. The one below returns the class. For example factors have mode \code{"numeric"} but class \code{"factor"}

<<exploring-dfs-3>>=
sapply(X = chickwts, FUN = class)
@

Each element of the vector is named according to the name of the corresponding ``column'' in the data frame. For this same statement to be used with any other data frame or list, we need only to substitute the name of the object, the argument to the first parameter called \code{X}, to the one of current interest.

\begin{playground}
Data sets \code{airquality} and \code{iris} contain data from air quality measurements in New York and for the size of flower parts of different species of irises. Load them, and repeat the steps above, to learn what variables (columns) it contains, their classes, the number of rows, etc.
\end{playground}
