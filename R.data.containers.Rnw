...
\begin{faqbox}{How to create an empty list?}
  In the same way as \code{numeric()} by default creates a \code{numeric} vector of length zero, \Rfunction(liste()) by default creates a \code{list} with no members.
