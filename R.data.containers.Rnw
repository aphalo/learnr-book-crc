...
The operators can be used together as shown below, with \code{lst1[[1]]} extracting the vector from \code{lst1} and \code{[3]} extracting the member at position 3 of the vector.
...
Compare them again, and try to understand why \code{lst2} has changed as it did. Pay also attention to possible changes to the members' names.
...
The list \code{lst10} is a nested system of lists, but all the ``terminal'' members are character strings. In other words, terminal nodes are all of the same mode, allowing the list to be ``flattened'' into a character vector.
...
Data frames are created with constructor function \Rfunction{data.frame()} with a syntax similar to that used for lists.
...
df1[ , 1]
# first column, df1[["x"]] or a.df$x preferred
df1[ , "treatment"]
# first row
df1[1, ]
