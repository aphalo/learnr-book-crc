...
When\index{data frames!summarizing} using \Rfunction{aggregate()}, the \emph{lhs} of the formula indicates the variable to summarize and the \emph{rhs} the factor used to split the data before summarizing them.
...
\begin{infobox}
Function \Rfunction{aggregate()} can be also used to aggregate time series data based on time intervals.
\end{infobox}
