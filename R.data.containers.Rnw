...
\subsection{Splitting and summarizing data frames}\label{sec:calc:df:subset:split}
\index{data frames!splitting}
\Rlang function \Rfunction{split()} makes it possible to split a data frame into a list of data frames, based on the levels of a factor.
...
When using \Rfunction{aggregate()}, the \emph{lhs} of the formula indicates the variable to summarize and the \emph{rhs} the factor used to split the data before summarizing them.
<<data-frames-7d>>=
...
If all the columns not used for grouping are valid input to the function passed as argument to \code{FUN} the formula can be simplified using \code{.} with meaning ``all columns except those on the \emph{rhs}''.
<<data-frames-7f>>=
