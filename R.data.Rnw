...
The authors of package \pkgname{tibble} describe as their class as backwards compatible with \code{data.frame} and make them a derived class. This backwards compatibility is incomplete 
...
How can \emph{pipes} exist within a single \Ralng script? Within \Rlang data sources and sinks are \Rlang objects and instead of programs being chained into a pipe, functions are chained. The connection between functions is through temporary \Rlang objects stored in memory instead of through temporary OS files stored on disk. As an abstraction there is no difference.
...
Although pipes are what make the scripts visually very different, from the point of view of data analysis what makes pipes most convenient to use are the new classes, functions and methods defined in package \pkgname{dplyr} and other \pkgname{tidyverse} packages. It is important to be aware the 
