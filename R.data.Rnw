...
For the reverse operation, converting from long form to wide form, we may try to use \Rfunction{pivot\_wider()}. However, the obvious simple code does not reconstruct the original data structure.
...
%
\begin{playground}
  Functions \Rfunction{pivot\_longer()} and \Rfunction{pivot\_wider()} from package \pkgname{poorman} attempt to replicate the behaviour of the same name functions from package \pkgname{tidyr}. In some edge cases, the behaviour differs. To test if the two code chunks above return identical or equal values when \code{poorman::}  is prepended to the names of these two functions. First, ensure than package \pkgname{poorman} is installed. At the time I typeset the book I had to modify
  the argument passed to \code{cols} to be a numeric vector instead of character vector.

<<tidy-tibble-01c, eval=eval_playground>>=
poor_long_iris.tb <-
  poorman::pivot_longer(
    iris,
    cols = which(colnames(iris) %in% setdiff(colnames(iris), "Species")),
    names_to = "part",
    values_to = "dimension")
