...
\begin{infobox}
The data sets currently used in this chapter are at the moment avaialble for download. The details of how to download files from within R is explained in section \ref{sec:files:remote} on page \pageref{sec:files:remote}. The examples for local data use the same files. As it is easier to first examplify reading local files, please, run the code in the chunk below at leats once, before attempting to run the code in the next sections. Make sure that the current folder/directory is the same one that will be current when running the examples.
