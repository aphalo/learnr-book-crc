...
<<tidy-tibble-01>>=
head(iris, 2)
long_iris <- gather(iris, key = part, value = dimension, -Species)
head(long_iris)
@
...
spread(long_iris, key = c(!!part, Species), value = dimension) # does not work!!
@
...
<<tidy-tibble-06b>>=
select(iris, Species, matches("pal"))
@
