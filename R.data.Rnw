...
\chapter{Data: from import to report}\label{chap:R:data}
...
Base \Rlang and the recommended packages (installed by default) include many functions for importing and/or manipulating data. This is a complete set, that supports all the usually needed operations. However, many of these functions have not been designed to perform optimally on very large data sets \autocite[see][]{Matloff2011}. The \Rlang implementation has been over the years improved significantly in performance. However, in some cases performance is constrained by the design of the \Rlang language itself. Contributed packages have aimed at improving performance and/or providing and enhanced set of operations and syntax.

One set of packages under the name of \pkgname{tidyverse} provides not only new functions but also new operators that allow a new way of assembling statements or ``sentences'' by chaining them into \emph{pipes} reminiscent of those available in Unix shells.

In this chapter you will learn how to import, manipulate and export data using the grammar of data as implemented in the `tidyverse' packages and how to use in this new approach base \Rlang functions and methods. As in previous chapters I will focus more on the available tools and how to use them than on their role in the analysis of data. The books \citebooktitle{Wickham2017} \autocite{Wickham2017} and \citebooktitle{Peng2016} \autocite{Peng2016} cover partly the same subjects from the perspective of data analysis and in more depth.
...
The operators needed to build pipes of \Rlang functions are implemented in package \Rpckage{magrittr}. This implementation is used in the \Rpackage{tidyverse} packages and some operators re-exported.
...
<<pipes-03>>=
data.out <- sum(sqrt(data.tmp))
@

written as a pipe using the chaining operator from package \Rpckage{magrittr}.
<<pipes-03>>=
data.in %>% sqrt() %>% sum() -> data.out
@

Although pipes are what make grammar of data scripts visually different, from the point of view of data analysis what makes pipes useful are the new classes, functions and methods defined in the different \Rpackage{tidyverse} packages. We will describe them following the usual flow of data through a data analysis pipe, starting with data input and output, and continuing in the next chapter with plotting.
