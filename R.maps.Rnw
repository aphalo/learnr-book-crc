...
\section{Using ImageMagick STL library}

Package \pkgname{magick} opens the door to using the \pgrmname{ImageMagick} library from within R. This package is quite new, but very promising, as \pgrmname{ImageMagick} is multi-threaded and can take advantage of processors with multiple cores. I will not describe here \pgrmname{ImageMagick}, you will need to consult its own documentation. In this section I will only show how some examples of how to meld its use into R code.
...
\section{Using ImageJ and Fiji}

Even more than \pgrmname{ImageMagick}, \pgrmname{ImageJ} will not really blend into R code. While \pgrmname{ImageMagick} exists as a library of functions designed to be easily called from other code, \pgrmname{ImageJ} is a program written in Java, that can be expanded by means of plugins. It is more visually oriented and frequently used interactively. With such a program, it makes most sense to use an IDE that integrates R and \pgrmname{ImageJ}, and allows easy exchange of data between them. \pgrmname{Bio7} is such an IDE. Once again, \pgrmname{ImageJ} has vast capabilities, and its own extensive documentation which will not be repeated here.

Package \pkgname{LeafArea} provides an R interface to \pgrmname{ImageJ}.
