...
Converting the image to gray scale is easy as it is an 8 bit per channel image.
...
The line above is not that easy to understand. What is going on is the \texttt{ecdf(dahlia02g.img)} returns a \emph{function} built on the fly, and then with the additional set of parentheses we call it, and then pass the result to the \texttt{plot()} method. It could be split as follows into three statements.
...
We can further check what is happening by printing the function.
...
\begin{description}
  ...
  \item[Advanced] As Medium but use package are \pkgname{Rcpp} to implement the ``glue'' code calling functions in the \prgmname{CImg} library in C++ so that the data is passed back and forth between R and compiled code only once.
\end{description}
