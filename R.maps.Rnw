...
Now we plot the world map of the coastlines, on a longitude and latitude scale, as a \code{ggplot} using \Rfunction{geom\_polygon()}.
...
Next we plot all the layers using the Robinson projection. This is still a \code{ggplot} object and consequently one can plot data on top of the map, being aware of the transformation of the scale needed to make the data location match locations in a map using a certain projection.
