...
<<apply-00a, eval=FALSE>>=
# not run
my.mean <- function(na.rm = FALSE, x) {
  mean(x, na.rm)
}
apply(x, MARGIN = 1, FUN = my.mean) # triggers error!!
@

One or more additional named arguments can be passed to the function to be applied, but these are invariant.
<<apply-00b>>=
apply(x, MARGIN = 1, FUN = mean, trim = 0.1)
@
...
<<apply-06>>=
set.seed(123456)
a.mat <- matrix(runif(6), ncol = 2)
mean_and_sd <- function(x, na.rm = FALSE) {
       c(mean(x, na.rm = na.rm),  sd(x, na.rm = na.rm))
    }
c.mat <- apply(X = a.mat, MARGIN = 1, FUN = mean_and_sd, na.rm = TRUE)
class(c.mat)
dim(c.mat)
c.mat
@
