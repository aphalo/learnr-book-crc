...
If you have written programs in other languages, it will feel natural to you to use loops (\Rloop{for}, \Rloop{while}, and \Rloop{repeat}) for many of the things for which in \Rlang one would normally use vectorization. In \Rlang, using vectorization when possible keeps scripts shorter and easier to understand (at least for those with experience in \Rlang). More importantly, as \Rlang is mostly an interpreted language, vectorized arithmetic tends to be faster than the use of explicit iteration. In recent versions of \Rpgrm, byte-compilation is used by default and loops may be compiled on the fly, which relieves part of the burden of repeated interpretation. However, even byte-compiled loops are usually slower to execute than efficiently coded vectorized functions and operators. Vectorized arithmetic specified using vectorized operators tends to be fast, but for operations that are frequent \Rlang also provides functions with even better performance.

I used as an example the computation of the differences between successive numbers in a vector, applied to vectors of lengths ranging from 10 to 100 million numbers (Figure \ref{fig:diff:benchmarks}). In relative terms the difference is huge between loops and vectorization for vectors with up 1000 numbers, but the total times are very short. For these vectors, pre-allocation of a vector to collect the results makes almost no difference and using the extraction operator \Roperator{[]} together with the minus \Roperator{-} is fastest. For longer vectors there is only a small advantage of using function \Rfunction{diff()} over this approach. For \Rloop{while} and \Rloop{for} loops pre-allocation of the vector to collect results makes an important difference, especially in the second case. However, vectorized operators and functions remain a lot faster than explicit loops. Because of the computation, \Rfunction{apply()} and relatives cannot be used for this example.
...
\caption[Benchmark results for different approaches to computing running differences.]{Benchmark results for different approaches to computing running differences in numeric (double) vector of different lengths. The script used is included in the \pkgname{learnrbook} \Rlang package. The data in this figure were obtained in a computer with a Xenon processor with four cores and 32 GB of RAM, Windows 10 and \Rpgrm 4.3.1.}\label{fig:diff:benchmarks}
\end{figure}

Execution speed needs to be balanced against the effort invested in writing faster code. However, using vectorization and specific \Rlang functions requires little effort once one is familiar with them.\qRloop{for}

\begin{explainbox}
When\label{box:vectorization:perf} working with large data sets, or many similar data sets, one needs to take performance into account. As vectorization usually also makes code simpler, it is good style to use vectorization whenever possible. For operations that are frequently used, \Rlang includes specific functions. It is thus important to consider not only vectorization of operators but also check for the availability of performance-optimized functions for specific cases. The chunks below show the code for the six approaches compared in Figure \ref{fig:diff:benchmarks}, where \code{a} was a numeric vector constructed with function \code{rnorm()}.
...
The simplest way of measuring the execution time of an \Rlang expression is to use function \Rfunction{system.time()}. See package \pkgname{microbenchmark}, used for the benchmarks shown, for better-time-resolution.
\end{explainbox}
...
\begin{table}
  \caption{Vectorized functions that can replace iteration loops.\vspace{1ex}}\label{tab:vectorized:functions}
  \centering
\noindent
\begin{tabular}{lll}
  \toprule
...
\Rlang provides several functions that can be used to avoid writing iterative loops in \Rlang. The most frequently used are taken for granted: \Rfunction{mean()}, \Rfunction{var()} (variance), \Rfunction{sd()} (standard deviation), \Rfunction{max()}, and \Rfunction{min()}. Replacing code implementing an iterative algorithm by a single function call simplifies the script's code and can make it easier to understand. These functions are written in \Clang and compiled, so even when iterative algorithms are used, they are fast. Table \ref{tab:vectorized:functions} lists additional functions available in base \Rlang that implement iterative algorithms is provided below. All these functions take a vector of arbitrary length as their first argument, except for \Rfunction{inverse.rle()}.\vspace{2ex}
