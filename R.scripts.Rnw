...
You can open a operating system's \emph{shell} from the Tools menu in \RStudio, to run this command. The output will be printed to the shell console. If you would like to save the output to a file, use redirection.
\begin{shaded}
\footnotesize
\begin{verbatim}
...
\code{repeat}\index{iteration!repeat loops} is seldom used, but adds flexibility as \Rfunction{break()} can be in the middle of the compound statement.
