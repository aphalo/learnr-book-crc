...
In addition to \Rcontrol{if} and \Rcontrol{if\ldots{}else}, there is in \Rlang a \Rcontrol{switch()} statement (Figure \ref{fig:switch:diagram}). It can be used to select among several \emph{cases}, or alternative statements, based on an expression that returns a \code{numeric} or a \code{character} value of length one when evaluated.
