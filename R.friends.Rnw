...
If when developing software you hit some apparent limit, then think laterally. Explore outside the box. A pair of very inspiring books by Jon Bentley (\citeyear{Bentley1986,Bentley1988}) are a very good starting point, even though they make no mention to R. As the quotation at the top of this page states, the surest way to improve performance is to use better algorithms. However, sometimes the \langname{R} language, being interpreted, depresses performance. In addition, sometimes good implementations of very good algorithms are already available as libraries written in other computer languages.

\langname{S} (\langname{R} is sometimes called \textsf{Gnu S}\index{Gnu S|see {R}}) is a very well designed language, but it was designed with a specific purpose in mind. No tool can handle efficiently and gracefully every task one may want to throw at it. R is no different. However, R is good at working together with other languages and tools. As for most tasks in life, for data analysis one may find the best tools for our toolbox scattered around the place and coming from different sources. What is important in any tool pack is that all the tools can be used together in an efficient and `fluid' combination. In this final chapter, I will give an overview of what is available, giving only minimal examples, as a teaser for readers to explore on their own what may be most useful for their jobs.
