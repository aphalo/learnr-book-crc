...
Profiling tools such as those provided by package \pkgname{proftools} can be useful when dealing with both simple and complex pieces of code. Furthermore, for interactive use, package \pkgname{profvis} should be used instead of package \pkgname{proftools}.
