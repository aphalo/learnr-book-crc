...
\noindent
\emph{Learn R: As a Language} aims to teach the R language, rather than its use in specific fields and situations. Nowadays, many students of biological and environmental sciences learn R in courses about statistics or data analysis. However, in many cases not in enough depth to effectively use it in scripts for automating data analyses or documenting them to ensure reproducibility. Students in the humanities and also in other fields, may find it easier to learn the R language separately from data analysis and statistics. A book with a focus on the language can later serve as a reference, and using as reference a book that one is already familiar with, tends to be very effective.
...
The key to the marginal bars and icons is given next. You should not take them too seriously, they are guidelines based on my own idea of what may be difficult or not. Even if reading through the whole text, they will warn about what content is more advanced.
...
One approach to reading the book is on first reading to read only the parts without any markings, which matches those books that skip most of the difficult details. What I think is most useful, is not to skip the description of unusual features and special cases, but rather to skim enough from them so that you have an idea of what special situations you may face when using R. Once you have read the book once, I expect it to serve as a reference, both for refreshing the mainstream features and to deal with the oddities and quirks of the language. To make use as a reference easy, I have been thorough with the indexing, including many carefully chosen terms and their synonyms and all R objects and constructs discussed, collecting them in three alphabetical indexes: \emph{General index}, \emph{Index of R names by category}, and \emph{Alphabetic index of R names}.
...
R code is typeset in a typewriter font, and using colour to highlight the different elements of the syntax, such as variables, functions, constant values, etc. For example in the ``code chunk'' below \code{mean()} and \code{print()} are functions; 1, 5 and 3 are constant numeric values, and \code{z} is the name of a variable where the result of the computation done in the first line of code is stored. The line starting with \code{\#\# } shows what is printed to the screen when executing the second statement: \code{[1] 1}.
...
In many ways this text owes much more to people who are not authors than to myself. However, as I am the one who has written this version and decided what to include and exclude, as author, I take full responsibility for any errors and inaccuracies.
%\newpage
