...
\index{classes and modes|(}\qRclass{numeric}\index{math operators}\index{math functions}\index{numeric values}
...
\index{classes and modes!numeric|)}

\section{Logical values and Boolean algebra}\label{sec:calc:boolean}
\index{classes and modes!logical|(}\index{logical operators}\index{logical values}\index{Boolean arithmetic}
...
It can be seen above that subsetting does not drop unused factor levels, and that \code{factor()} can be used to explicitly drop the unused factor levels.\index{factor!dropping unused levels}
...
\begin{explainbox}
\textbf{Why is a double conversion needed?}\index{factors!conversion to numeric} Internally factor levels are stored as running integers starting from one, and those are the numbers returned by \Rfunction{as.numeric()} when applied to a factor. The labels of the factor levels are always stored as character strings, even when these characters are digits. In contrast to \Rfunction{as.numeric()}, \Rfunction{as.character()} returns the character labels of the levels. If these character strings represent numbers, they can be converted, in a second step, using \Rfunction{as.numeric()} into the original numeric values.
\end{explainbox}
...
\begin{explainbox}
\textbf{Renaming factor levels.}\index{factors!renaming levels} Most direct way is using \Rfunction{levels() <-} as shown below, but it is also possible to use \Rfunction{factor()}. The difference is that \code{factor()} drops the unused levels and \code{levels()} only renames existing levels. All of them.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
Or only some by name.
<factors-2b>>=
my.factor1 <- gl(4, 3, labels = c("A", "F", "B", "Z"))
my.factor1
levels(my.factor1) <- c(A = "a", Z = "abc")
my.factor1
@

Or alternatively by position---i.e.\ rather unsafe.
<factors-2c>>=
my.factor1 <- gl(4, 3, labels = c("A", "F", "B", "Z"))
my.factor1
levels(my.factor1)[1, 4] <- c("a", "abc")
my.factor1
@
...
\begin{explainbox}
\textbf{Merging factor levels.}\index{factors!merging levels} We use \Rfunction{factor()} as shown below, setting the same label for the levels we want to merge.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{my.factor1} \hlkwb{<-} \hlkwd{gl}\hlstd{(}\hlnum{4}\hlstd{,} \hlnum{3}\hlstd{,} \hlkwc{labels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"A"}\hlstd{,} \hlstr{"F"}\hlstd{,} \hlstr{"B"}\hlstd{,} \hlstr{"Z"}\hlstd{))}
...
\hlkwd{factor}\hlstd{(my.factor1,}
       \hlkwc{levels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"A"}\hlstd{,} \hlstr{"F"}\hlstd{,} \hlstr{"B"}\hlstd{,} \hlstr{"Z"}\hlstd{),}
       \hlkwc{labels} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"A"}\hlstd{,} \hlstr{"C"}\hlstd{,} \hlstr{"B"}\hlstd{,} \hlstr{"C"}\hlstd{))}
\end{alltt}
\begin{verbatim}
...
\end{alltt}
\begin{verbatim}
##  [1]  7.573675  7.836288 10.634097  9.458268 12.887823  7.306056  8.859957
##  [8]  9.173986 10.313441  7.493926
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\begin{verbatim}
##  [1] 0.7284414 0.6308598 0.4107011 0.3568537 0.5993450 0.4885687 0.4752139
##  [8] 0.5642526 0.3947650 0.4037925
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\begin{verbatim}
##  [1] 0.7820728 0.6347158 0.3882744 0.3332653 0.6192666 0.4733070 0.4726155
##  [8] 0.5860681 0.3968811 0.4179825
\end{verbatim}
\end{kframe}
\end{knitrout}
