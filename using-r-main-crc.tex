...
Depending on the characteristics of the data being mapped scales can be continuous or discrete. There are scales available for different attributes of the plotted geometrical object, such as position (\code{x, y, z}), \code{size}, \code{shape}, \code{linetype}, \code{colour}, \code{fill}, \code{alpha} or transparency, \code{angle}, that the grammar of graphics calls \emph{aesthetics}. Scales determine how values in data are mapped to values of an \emph{aesthetics}, and how these values are labelled.

Some \emph{aesthetics} like \code{size} can vary continuously but others like \code{linetype} are inherently discrete. In addition to discrete scales for inherently discrete\emph{aesthetics}, discrete scales are available for those \emph{aesthetics} that are inherently continuous, like \code{x}, \code{y}, \code{size}, \code{colour}, etc.\

The scales used by default set the mapping automatically (e.g.\ which colour corresponds to $x = 0$ and which one to $x = 1$). However, for each \emph{aesthetic} such as \code{colour} there are multiple scales, both continuous and discrete, to choose from when creating a plot (e.g.\ 20 different colour scales in \ggplot 3.2.0).

\begin{explainbox}
Aesthetics in a plot layer in addition to being determined by mappings, can be also set to a constant value (e.g.\ setting all symbols in a layer created with \code{geom\_point()} to be red instead of the default of black). Aesthetics set to constant values, are not mapped to data, and consequently independent of scales. In other words properties of plot elements can be either set to a single constant value of an \emph{aesthetic} affecting all observations present in \code{data}, or mapped to a variable in \code{data} in which case the mapped aesthetic will reflect the values stored in \code{data} for the mapped variable.
\end{explainbox}

The most direct mapping to data is \code{identity}, which means that the data is taken at its face value. In a colour scale, say \ggscale{scale\_colour\_identity()}, the variable in the data would be encoded with values such as \code{"red"}, \code{"blue"}---i.e. valid \Rlang colours. In a simple mapping using \ggscale{scale\_colour\_discrete()} levels of a factor, such as \code{"treatment"} and \code{"control"} would be represented as distinct colours with the correspondence of individual levels to individual colours selected automatically by default. In contrast with \code{scale\_colour\_manual()} the user needs to explicitly provide the mapping between factor levels and colours by passing arguments to the scale functions' parameters \code{breaks} and \code{values}.
...
\index{plots!tag|(}
We here describe a feature common to all scales, the \code{name}. The default \code{name} of all scales is the name of the variable or the expression mapped. In the case of the \code{x}, \code{y} and \code{z} \emph{aesthetics} the \code{name} given to the scale is used for the axis labels. For other \emph{aesthetics} the name of the scale becomes the ``heading'' of the guide or key. All scales have a \code{name} parameter to which a character string or \Rlang expression (see section \ref{sec:plot:plotmath}) can be passed as argument to override the default.

Whole-plot title, subtitle and caption are not connected to scales, but convenience function \code{labs()} allows to set both these and the \code{name} of scales. A \code{title}, \code{subtitle}, \code{caption} and \code{tag} can be added with function \Rfunction{ggtitle()} by passing either character strings or \Rlang expressions as arguments.
...
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= Orange,}
  ...
  \hlkwd{ggtitle}\hlstd{(}\hlkwc{title} \hlstd{=} \hlstr{"Growth of orange trees"}\hlstd{,}
...
There are convenience functions for setting the axis labels, \Rfunction{xlab()} and \Rfunction{ylab()}. The code below can replace the two scales in the chuck above.
...
Function \Rfunction{labs()} is a convenience function useful when we use the default scales for all the \emph{aesthetics} in a plot but we want to manually set axis labels and/or key titles. \Rfunction{labs()} accepts for scales, arguments named according to the names of the \emph{aesthetics}, or using the names of the parameters of \code{ggtitle()} as used above.

\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
  \hlkwd{labs}(title = \hlstr{"Growth of orange trees"},
       subtitle = \hlstr{"Starting from 1968-12-31"},
       caption = \hlstr{"see Draper, N. R. and Smith, \hlkwd{H.} (1998)"},
       tag = \hlstr{"A"},
       x = \hlstr{"\hlkwd{Time} (d)"},
       y = \hlstr{"\hlkwd{Circumference} (mm)"},
\end{alltt}
\end{kframe}
\end{knitrout}
...
Limits are relevant to all kinds of scales. Limits are set through parameter \code{limits} of the different scale functions. They can be also set with convenience functions \code{xlim()} and \code{ylim()} in the case of the \code{x} and \code{y} aesthetics, and more generally with function \code{lims()} which analogously to \code{labs()} takes arguments named according to the name of the \emph{aesthetics}. The \code{limits} argument of scales accepts vectors, factors or a function computing them from the data. In contrast, the convenience functions do not accept functions as their arguments.

In the next example we set ``hard'' limits, which will exclude some observations from those included in the plot and from any calculations of summaries or smoothers. More exactly, the off limits observations are converted to \code{NA} values.
...
In general setting hard limits should be avoided, even though a warning is issued when observations are omitted, as it is easy to unwillingly subset the data being plotted.
It is preferable to use function \Rfunction{expand\_limits()} makes it possible to safely \emph{expand} the dynamically computed default limits of a scale---in this case the scale limits will grow past the expanded values when needed to accommodate all observations. The arguments to \code{x} and \code{y} are numeric vectors of length one or two each, matching how the limits of the $x$ and $y$ continuous scales are defined. Here we expand the limits to include the origin.
...
In the case of scales, we cannot reverse their direction through the setting of limits. We need instead to use a transformation as described in section \ref{sec:plot:scales:trans} on page \pageref{sec:plot:scales:trans}.
...
The\index{plots!scales!continuous!transformations} default scales used by the \code{x} and \code{y} aesthetics, \ggscale{scale\_x\_continuous()} and \ggscale{scale\_y\_continuous()}, use \code{trans = "identity"} by default and accept a transformation function as argument to parameter \code{trans}. In addition there are predefined convenience scale functions for \code{log10}, \code{sqrt} and \code{reverse}.
...
Using a transformation in a scale is not equivalent to applying the same transformation on-the-fly when mapping it to the $x$ \emph{aesthetic} as this results in tick-labels expressed in transformed values.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
The default position of axes can be changed through parameter \code{position}.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
It also possible to use different \code{breaks} and \code{labels} than the main axes, and to provide a \code{name} to be used as axis label,
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\index{color!using|(}
Colors can be specified in \Rlang not only through character strings with the names of previously defined colors, but also directly as strings describing the RGB (red, green and blue) components as hexadecimal numbers (on base 16) such as \code{"\#FFFFFF"} for white or \code{"\#000000"} for black, or \code{"\#FF0000"} for the brightest available pure red. The list of color names known to \Rlang can be obtained be entering \code{colors()} in the console.

Given the number of colors available, we may want to subset them based on their names. Function \code{colors()} returns a character vector. We can use \code{grep()} or \code{grepl()} to find the names containing a given character substring, in this example \code{"dark"}.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
With function \Rfunction{rgb()} we can define new colors.
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
{\centering \includegraphics[width=.54\textwidth]{figure/pos-main-chunk-20-1} 
...
{\centering \includegraphics[width=.54\textwidth]{figure/pos-main-chunk-21-1} 
\includegraphics[width=.54\textwidth]{figure/pos-main-chunk-21-2} 
...
\includegraphics[width=.54\textwidth]{figure/pos-main-chunk-27-2} 
\includegraphics[width=.54\textwidth]{figure/pos-main-chunk-27-3} 
...
{\centering \includegraphics[width=.54\textwidth]{figure/pos-main-chunk-28-1} 
...
## [37] crayon_1.3.4       withr_2.1.2        sf_0.7-4          
## [40] MASS_7.3-51.4      grid_3.6.0         nlme_3.1-139      
## [43] jsonlite_1.6       gtable_0.3.0       DBI_1.0.0         
## [46] formatR_1.7        magrittr_1.5       units_0.6-3       
## [49] scales_1.0.0       KernSmooth_2.23-15 cli_1.1.0         
## [52] stringi_1.4.3      reshape2_1.4.3     farver_1.1.0      
## [55] tidyverse_1.2.1    xml2_1.2.0         generics_0.0.2    
## [58] RColorBrewer_1.1-2 learnrbook_0.0.2   glue_1.3.1        
## [61] beeswarm_0.2.3     tweenr_1.0.1       hms_0.4.2         
## [64] colorspace_1.4-1   filehash_2.4-2     classInt_0.3-3    
## [67] rvest_0.3.4        haven_2.1.0
\end{verbatim}
\end{kframe}
\end{knitrout}
