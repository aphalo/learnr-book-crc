...
\usepackage{tikz}
\usetikzlibrary{shapes.geometric,shapes.symbols,positioning,fit,arrows,matrix,backgrounds}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{a.list} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlkwc{x} \hlstd{=} \hlnum{1}\hlopt{:}\hlnum{6}\hlstd{,} \hlkwc{y} \hlstd{=} \hlstr{"ab"}\hlstd{,} \hlkwc{z} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{TRUE}\hlstd{,} \hlnum{FALSE}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\matrix[array] (array) {
1 & 2 & 3 \\
1:6\strut & ``ab''\strut & c(TRUE, FALSE)\strut \\};
