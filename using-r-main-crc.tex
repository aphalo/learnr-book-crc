...
\begin{table}
  ...
  \centering
   \begin{tabular}{llp{8.25cm}}
     ...
     \code{geom\_plot} & \pkgnameNI{ggpp} & x, y, label, angle \\
     \code{geom\_grob} & \pkgnameNI{ggpp} & x, y, label \\
...
\index{plots!scatter plot|(}The first example produces a scatter plots, because numeric variables are mapped to both \code{x} and \code{y}.

\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.984, 0.984, 0.984}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,}
       \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= disp,} \hlkwc{y} \hlstd{= mpg,} \hlkwc{shape} \hlstd{=} \hlkwd{factor}\hlstd{(cyl)))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()}
\end{alltt}
\end{kframe}
...
\begin{table}
  \caption[\ggplot statistics]{\ggplot statistics described in section \ref{sec:plot:statistics}, packages where they are defined, their default geometry and the aesthetics they use as input for computations.}\vspace{1ex}\noindent\label{tab:plot:stats}
  \centering
   \begin{tabular}{llll}
     \toprule
     ...
     \code{stat\_poly\_line} & \pkgnameNI{ggpp} &\code{geom\_smooth} & x, y, weight \\
     \code{stat\_poly\_eq} & \pkgnameNI{ggpp} & \code{geom\_text} & x, y, weight  \\
     \code{stat\_fit\_tb} & \pkgnameNI{ggpp} & \code{geom\_table} & x, y, weight  \\
     ...
     \code{geom\_violin} & \pkgnameNI{ggplot2} & --- & x, y, weight  \\
     \code{geom\_quasirandom} & \pkgnameNI{ggbeeswarm} & --- & x, y \\
     \code{stat\_ma\_line} & \pkgnameNI{ggpmisc} & \code{geom\_smooth} & x, y \\
     \code{stat\_ma\_eq} & \pkgnameNI{ggpmisc} & \code{geom\_text} & x, y, weight  \\
     \code{stat\_centroid} & \pkgnameNI{ggpmisc} & \code{geom\_point} & x, y \\
     \code{stat\_quant\_line} & \pkgnameNI{ggpmisc} & \code{geom\_smooth} & x, y, weight  \\
     \code{stat\_quant\_eq} & \pkgnameNI{ggpmisc} & \code{geom\_text} & x, y, weight \\
