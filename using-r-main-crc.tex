...
\begin{warningbox}
When a ggplot is built, the whole plot and its components are created as \Rlang objects that can be saved in the workspace or written to a file as objects. These objects encode a recipe for constructing the plot, not its final graphical representation. The graphical representation is generated when the object is printed, explicitly or automatically. Thus, the same \code{"gg"} plot object can be rendered into different bitmap and vector graphic formats for display or printing.
\end{warningbox}
...
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,}
       \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= disp,} \hlkwc{y} \hlstd{= mpg))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{color} \hlstd{=} \hlstr{"blue"}\hlstd{,} \hlkwc{shape} \hlstd{=} \hlstr{"square"}\hlstd{))}
\end{alltt}
\end{kframe}
...
We can manipulate \code{"gg"} plot objects and their components in the same way as other \Rlang objects. We can operate on them using the operators and methods defined for the \code{"gg"} class they belong to. We start by saving, similarly as above, a \code{"gg"} plot object in variable \code{p1}.
...
The last two examples above show two different approaches. When plots have a single layer, they are equally convenient. If a plot has multiple layer based on the same data, the first of this approaches makes this clear. If each layer uses different data, the second approach is better than using a combination of the two approaches. As discussed earlier, using a consistent and clear style when coding is important.

\begin{explainbox}
In some cases, when flexibility is needed while constructing complex plots with multiple layers other idioms can be preferable, e.g., when assembling a plot from ``pieces´´ stored in variables or built programmatically. Some examples are given below.
...
In all these examples, the plot remains unchanged (not shown). However, the flexibility of the grammar allows the assembly of plots from separately constructed pieces and reusing these pieces by storing them in variables.
\end{explainbox}

The mapping of aesthetics does not necessarily have to be to variable in \code{data}, it can be a code statement that returns a value computed from one of more variables in \code{data} and constant values. In this simple example, \code{mpg} in miles per gallon is first converted into km per litre.

\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.984, 0.984, 0.984}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{my.mapping} \hlkwb{<-} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= disp,} \hlkwc{y} \hlstd{= mpg} \hlopt{*} \hlnum{0.43}\hlstd{)}
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,}
       \hlkwc{mapping} \hlstd{= my.mapping)} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()}
\end{alltt}
...
Subsetting of data is in principle not possible within a call to \Rfunction{aes()}, but see below. When a plot has default data, the argument passed to parameter \code{data} of a layer function, can be a function returning a data frame. In this case, the function is applied to the default data and the data frame it returns used in the layer. In the example below an anonymous function defined in-line, extracts a subset of rows in data and highlights it by overplotting with a different color. The argument passed to data is a function definition, not a call, thus, if passed by name, no parentheses are used.
...
The plot's default data can also be operated upon using the \pkgname{magrittr} pipe operator, but not the pipe operator native to \Rlang (\Roperator{\textbar >}) or the dot-pipe operator from \pkgname{wrapr} (see section \ref{sec:data:pipes} on page \pageref{sec:data:pipes}). In my opining, using a function as above is simpler and clearer.
...
For this example, an alternative approach is to put the test condition within the call to \Rfunction{aes()}. Setting \code{size = 0} draws a very small point, but visible. Using \code{NA} instead, skips the rows, and \code{na.rm = TRUE} silences the warning. This is a roundabout approach to subsetting.
...
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,}
  ...
  \hlkwd{geom_point}\hlstd{(}\hlkwc{color} \hlstd{=} \hlstr{"yellow"}\hlstd{,}
             \hlkwd{aes}\hlstd{(}\hlkwc{size} \hlstd{=} \hlkwd{ifelse}\hlstd{(cyl} \hlopt{==} \hlnum{4}\hlstd{,} \hlnum{1.5}\hlstd{,} \hlnum{NA}\hlstd{)),}
             \hlkwc{na.rm} \hlstd{=} \hlnum{TRUE}\hlstd{)} \hlopt{+}
  \hlkwd{scale_size_identity}\hlstd{()}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=.7\textwidth]{figure/pos-ggplot-basics-18-1} 
...
%
The documentation of \pkgname{ggplot2} gives several good examples of cases when the new syntax is useful. I give here a different example. A polynomial fitted to data using \Rfunction{rlm()}. RLM is a procedure that automatically assigns before computing the residual sums of squares, weights to the individual residuals in an attempt to protect the estimated fit from the influence of extreme observations or outliers. When using this and similar methods it is of interest to plot the residuals together with the weights. A frequent approach is to map weights to a gradient between two colours. The code below constructs a data frame containing artificial data that includes an extreme value or outlier.
...
\definecolor{shadecolor}{rgb}{0.984, 0.984, 0.984}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{ggplot}\hlstd{(df2,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= X,} \hlkwc{y} \hlstd{= Y))} \hlopt{+}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: The `scale\_name` argument of `continuous\_scale()` is deprecated as of ggplot2\\\#\# 3.5.0.\\\#\# This warning is displayed once every 8 hours.\\\#\# Call `lifecycle::last\_lifecycle\_warnings()` to see where this warning was\\\#\# generated.}}\end{kframe}
