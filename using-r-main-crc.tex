...
Instead of making the points semitransparent, we can randomly displace them to avoid overlaps. This is called \emph{jitter}, and can be added using \ggposition{position\_jitter()} as argument to formal parameter \code{position} and the amount of jitter set with \code{width} as a fraction of the distance between adjacent factor levels in the plot. The name as a character string can be also used when no arguments need to be passed to the \emph{position} function.
...
\begin{explainbox}
The displacement introduced by jitter and nudge differ in that jitter is random, and nudge deterministic. \gggeom{geom\_point\_s()} from package \pkgname{ggpp} is used to make the displacement visible by drawing an arrow connecting original and displaced positions for each observation. \ggposition{position\_identity()} is the default for \gggeom{geom\_point()}. Position functions are passed as argument to formal parameter \code{position} of \emph{geometries}.
...
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{=} \hlkwd{factor}\hlstd{(cyl),} \hlkwc{y} \hlstd{= mpg))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_point_s}\hlstd{(}\hlkwc{position} \hlstd{=} \hlkwd{position_jitter}\hlstd{(}\hlkwc{width} \hlstd{=} \hlnum{0.33}\hlstd{),}
               \hlkwc{colour} \hlstd{=} \hlstr{"red"}\hlstd{,}
               \hlkwc{colour.target} \hlstd{=} \hlstr{"all"}\hlstd{)}
\end{alltt}
\end{kframe}
...
\hlkwd{ggplot}\hlstd{(}\hlkwc{data} \hlstd{= mtcars,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{=} \hlkwd{factor}\hlstd{(cyl),} \hlkwc{y} \hlstd{= mpg))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_point_s}\hlstd{(}\hlkwc{position} \hlstd{=} \hlkwd{position_nudge}\hlstd{(}\hlkwc{x} \hlstd{=} \hlnum{0.5}\hlstd{,} \hlkwc{y} \hlstd{=} \hlnum{2}\hlstd{),}
               \hlkwc{colour} \hlstd{=} \hlstr{"red"}\hlstd{,}
               \hlkwc{colour.target} \hlstd{=} \hlstr{"all"}\hlstd{)}
\end{alltt}
\end{kframe}
