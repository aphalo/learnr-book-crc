...
I also use the notation \textcolor{blue}{\code{<value>}},  \textcolor{blue}{\code{<statement>}}, etc., as a generic placeholder in diagrams indicating \emph{any valid value}, \emph{any valid R statement}, etc.

R code is typeset using colour to highlight the different elements of the syntax, such as variables, functions, constant values, etc. For example in the ``code chunk below'' \code{mean()} and \code{print()} are functions; 1, 5 and 3 are constant values, and \code{z} is the name of a variable where the result of the computation done in the first line of code was stored. The line starting with \code{\#\# } shows what is printed to the screen when executing the last statement: \code{[1] 1}.
