\documentclass[paper=a4,10pt,div=17,headsepline,BCOR=12mm,twoside,open=right]{scrbook}
...
opts_fig_wide_square <- list(fig.width=6, fig.height=6, out.width='.63\\textwidth')
...
\chaper*{Learning like a child}

Do not struggle, just play! If going gets difficult and frustrating, take a break! If you get a new insight, take a break to enjoy the victory! Such pauses are not a miss use of our time. To learn a natural language we need to interact with other speakers, we need feedback. In the case of R, we can get feedback both from the outcomes from our utterances to the computer, and from other R users.
...
<<child-r-data, child='R.data.Rnw', eval=FALSE>>=
@
