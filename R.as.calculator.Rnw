...
\begin{explainbox}
  When dealing with deep lists, it is sometimes useful to limit the number of levels of nesting returned by \code{str()} by means of a \code{numeric} argument passed to parameter \code{max.levels}.
