...
Comparison operators return vectors of \code{logical} values (see section \ref{sec:calc:boolean} on page \pageref{sec:calc:boolean}),
with values \code{TRUE} or \code{FALSE} depending on the outcome.
...
\begin{explainbox}
Obviously binary values can be also stored as 0's and 1's, or as any other two numbers, in a \code{numeric} vector, or as text strings in a \code{character} vector. The point of using \Rclass{logical} values is that they support the operations of the Boolean algebra.

For example to test if members of a numeric vector are within a range, in our example, -1 to +1, we can combine the results from two comparisons using the vectorized logical AND operator \Roperator{\&}.
...
Or we can combine another two comparisons using the vectorized logical OR operator \Roperator{\textbar}.
...
\subsection{Character matching and substitution}\label{sec:calc:character:oper}
As\index{regular expressions}\index{character strings!partial matching and substitution} in \Rlang each character value is a string comprised by zero to many characters, in addition to comparisons based on whole strings or values, partial matches among them are of interest. An approach used in \Rlang and other languages is to use \emph{regular expressions} to concisely describe conditions for matching character strings. Regular expressions themselves are encoded as character strings, where some characters and character sequences have special meaning.
...
TODO: add examples with gsub(), grep(), grepl(), etc.
