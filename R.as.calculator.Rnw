...
\draw (array-2-1.west) node [left] (first) {\textcolor{blue}{\code{<name>}}};
...
\draw (array-2-1.west) node [left] (first) {\code{\ a\ }};
...
\draw (array-2-1.west) node [left] (first) {\code{\ b\ }};
\draw (array-1-3.east)--++(0:3mm) node [right]{\code{integer} indices};
\draw (array-2-3.east)--++(0:3mm) node [right]{\code{numeric} values};
%
\end{tikzpicture}
...
\draw (array-2-1.west) node [left] (first) {\code{\ c\ }};
...
\draw (array-2-1.west) node [left] (first) {\code{\ d\ }};
\draw (array-1-6.east)--++(0:3mm) node [right]{\code{integer} indices};
\draw (array-2-6.east)--++(0:3mm) node [right]{\code{numeric} values};
%
\end{tikzpicture}
...
<<numbers-4ee>>=
c(d, numeric()) # concatenate d with a vector of length zero
@
...
\node [align=center, anchor=south] at (array-2-1.south west|-first.south) (1) [below]{\code{a[2]}};
\draw (1)--(box);
%
\end{tikzpicture}
\end{small}
...
\emph{Lists'} main difference from vectors is, in \Rlang, that they can be heterogeneous. In \Rlang, the members of a list can be considered as following a sequence, and accessible through numerical indexes, the same as members of vectors. Members of a list as well as members of a vector can be named, and retrieved (indexed) through their names. In practice, named lists are more frequently used than named vectors. Lists are created using function \Rfunction{list()} similarly as \Rfunction{c()} is used for vectors. Members of a list can be vectors differing both in their class and in their length, as well as other lists.
...
Lists can behave as vectors with heterogeneous elements as members, as we will first describe. As lists can be nested, tree-like structures are also possible (see section \ref{sec:calc:lists:nested} on page \pageref{sec:calc:lists:nested}). Differently to languages like \Clang, \Cpplang and \pascallang, \Rlang does not implement memory pointers or linked lists.
