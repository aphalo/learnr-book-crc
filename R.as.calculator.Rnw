...
When working at the command prompt, results are printed by default, but in other cases you may need to use the function \code{print()} explicitly. The examples here rely on the automatic printing.
...
\begin{explainbox}
Here I very briefly introduce the concept of \emph{mode} of an R object. In the case of \Rpgrm, numbers, belong to mode \code{numeric}. We can query if the mode of an object is \code{numeric} with function \code{is.numeric()}.
...
As in computers numbers can be stored in different ways, most computing languages allow the use of several different types of numbers. In most cases \Rpgrm's \code{numeric} can be used everywhere where a number is expected. In some cases in can be more efficient to explicitly indicate whether we will store or operate on integer numbers, in which case we can use class \code{integer}, with integer constants indicated with a trailing capital L, as in  \code{32L}. When the intention is to represent Real numbers, within a finite range, in other words, floats, we can directly use class \code{double}.
...
When applied to vectors, \code{signif} behaves slightly differently, it ensures that the value of smallest magnitude retains \code{digits} significant digits.
...
\begin{playground}
What does value truncation mean? Function \code{trunc()} truncates a numeric value, but it does not return an \code{integer}.
\begin{itemize}
  \item Compare the values returned by \code{trunc()} and \code{as.integer()} when applied to a floating point number, such as \code{12.34}. Check for the equality of values, and for the \emph{class} of the returned objects.
  \item Explore how \code{trunc()} and \code{ceiling()} differ. Test them both with positive and negative values.
  \item \textbf{Advanced} Use function \code{abs()} and operators \code{+} and \code{-} to recreate the output of \code{trunc()} and \code{ceiling()} for the different inputs.
  \item Can \code{trunc()} and \code{ceiling()} be considered type conversion functions in \langname{R}?
\end{itemize}
\end{playground}

\sloppy
Other functions relevant to the ``conversion'' of numbers and other values are \code{format()}, and \code{sprintf()}. These two functions return character strings, instead of \code{numeric} or other values, and are useful for printing output. One could think of these functions as advanced conversion functions returning formatted, and possibly combined and annotated, character strings. However, they are usually not considered normal conversion functions, as they are very rarely used in a way that preserves the original precision of the input values.

\begin{playground}
Function \code{format()} may be easier to use, in some cases, but \code{sprintf()} is more flexible and powerful. Those with experience in the use of the \langname(C) language will already know about \texttt{sprintf()} and its use of templates for formatting output. Look up the help pages for both functions, and practice, by trying to create the same output by means of the two functions.
\end{playground}
...
Factors are used for indicating categories, most frequently the factors describing the treatments in an experiment, or categories in a survey. They can be created either from numerical or character vectors. The different possible values are called \emph{levels}. Normal factors created with \code{factor} are unordered or categorical. \Rpgrm also defines ordered factors that can be created with function \code{ordered}.
...
To investigate the returned values, function \code{str} for \emph{structure} tends to help, especially when the lists have many members, as it prints more compact output, than printing the same list.
...
\begin{playground}
What do you expect the each of the statements to return? \emph{Before running the code} predict what value and of which mode each statement will return. You may use implicit, or explicit, calls to \code{print()}, or calls to \code{str()} to visualize the structure of the different objects.
<<lists-4, eval=FALSE>>=
c.list[c(1,2,1,3)]
c.list[1]
c.list[[1]][2]
...
\begin{explainbox}
Sometimes we need to flatten a list, or a nested structure of lists within lists. Function \code{unlist()} is what should be normally used in such cases.
...
\Rpgrm is an object oriented language, and objects belong to classes. With function \code{class} we can query the class of an object. As we saw in the two previous chunks lists and data frames objects belong to two different classes. However, their relationship is based on a hierarchy of classes. We say that class \code{data.frame} is derived from class \code{list}. Consequently, data frames inherit the methods and characteristics of lists, which have not been modified for data frames.
...
\begin{warningbox}
We mentioned above that indexing by name can be done either with double square brackets, \code{[[ ]]}, or with \code{\$}. In the first case the name of the variable or column is given as a character string, enclosed in quotation marks, or as a variable with mode \code{character}. When using \code{\$}, the name is entered as is, without quotation marks.
...
Both in the case of lists and data frames, when using square brackets, an exact match is required between the name set in the object and the name used for indexing. In contrast, with \code{\$} any unambiguous partial match will be accepted. For interactive use, partial matching is helpful in reducing typing. However, in scripts, and especially R code in packages it is best to avoid the use of \code{\$} as partial matching to a wrong variable present at a later time, e.g.\ when someone else revises the script, can lead to very difficult to diagnose errors. In addition, as \code{\$} is implemented by first attempting a match the name and then calling \code{[[ ]]}, using \code{\$} for indexing can result in slightly slower performance compared to using \code{[[ ]]}.
\end{warningbox}

When the names of data frames are long, complex conditions become awkward to write using indexing---i.e.\ subscripts. In such cases \code{subset} is handy because evaluation is done in the `environment' of the data frame, i.e.\ the names of the columns are recognized if entered directly when writing the condition.
