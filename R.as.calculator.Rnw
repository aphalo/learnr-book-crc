...
As it can be seen in the first line of code above, another peculiarity of \Rpgrm, is what is frequently called ``recycling'' of arguments:\index{recycling of arguments} as vector \code{a} is of length \Sexpr{length(a)}, but the constant 1 is a vector of length 1, this short constant vector is extended, by recycling its value, into a longer vector of ones---i.e., a vector of the same length as the longest vector in the statement, \code{a}.\label{par:recycling:numeric}

Make sure you understand what calculations are taking place in the chunk above, and also the one below. Vector recycling is a key feature of the \Rlang language.
...
<<>>=
z <- numeric(length = 0) # named argument
z
length(z)
@

<<>>=
z1 <- numeric() # default argument
z1
@
