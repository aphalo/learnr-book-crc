...
\Attention{BROKEN} The code in the chunks below has stopped working with recent package updates. Some examples in other package vignettes seem to be also triggering errors. I need to investigate if this is due to ggplot2 $>=$ 2.2.1.9000. We temporarily disable evaluation of the two code chunks bellow.

<<ggnetwork-02, eval=FALSE>>=
set.seed(12345)
...
The \code{geom\_table} plots a data frame or tibble, nested in a tibble pased as data
...
the centre. This behaviour is the same as for justification in \code{geom\_text()} and 
\code{geom\_label()}.
...
\code{geom\_table} behaves like a ``normal'' ggplot geometry and that a table can be the only
...
\begin{explainbox}
  ...
  table themes is not yet supported by this geometry. Neither is the addition of multiple tables with a single call to \code{geom\_table} by passing a \code{tibble} with multiple rows as argument. These features will be added in a future version.
\end{explainbox}
...
\Attention{BROKEN} Function \Rfunction{coord\_tern()} and consequently also \Rfunction{ggtern()} trigger fatal error under `ggplot2' $\leq$ 2.2.1.9000. We temporarily disable evaluation of the code chunck bellow.
