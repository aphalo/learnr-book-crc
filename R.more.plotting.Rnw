...
Package \pkgname{ggbiplot}\index{plots!principal components} defines two functions, \code{ggscreeplot()} and \code{ggbiplot()}. These functions make it easy to nicely print the results from principal components analysis done with \code{prcomp}.
...
Sometimes it is useful to add marginal plots\index{plots!marginal}\index{marginal plots}\index{marginal histograms}\index{marginal density plots} to a \code{ggplot}. Package \pkgname{ggExtra} provides this functionality through an easy to use interface.
