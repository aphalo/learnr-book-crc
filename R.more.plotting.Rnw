...
The examples that follow using Google fonts are portable, but those using system fonts require the fonts to be available when the ggplot object is created.
