...
\section[waffle]{\pkgname{waffle}}\label{sec:plot:waffle}
\index{plots!square pie charts}
\index{plots!pie charts!square}
``Square pie charts''.

\section[ggmosaic]{\pkgname{ggmosaic}}\label{sec:plot:ggmosaic}
\index{plots!mosaic plots}
Mosaic plots. Coming soon.
...
The focus of this extension to \pkgname{ggplot2} is the conversion of time series data into tidy tibbles. It also defines additional \emph{geometries} for plotting moving averages with \ggplot. Package \pkgname{tidyquant} defines six \emph{geometries}, several mutators for time series stored as tibbles are also exported.
