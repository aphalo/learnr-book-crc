...
Finally a more complicated example, taking advantage of some of the flexibility that can be obtained by means of additional arguments.
...
\begin{warningbox}
\textbf{Known bug.} Function \ggcoordinate{coord\_tern()} and consequently also \Rfunction{ggtern()} trigger fatal error under `ggplot2' $\leq$ 2.2.1.9000. We temporarily disable evaluation of the code chunck bellow.
\end{warningbox}
