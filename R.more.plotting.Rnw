...
\begin{explainbox}
  This \emph{geometry} uses internally functions from package \pkgname{gridExtra}.
\end{explainbox}
...
the specification of the position and justification of the table is no longer intuitive.
...
It is also possible to rotate the table using \code{angle}.
