...
The examples that follow, using Google fonts are more portable as long that internet access is available.

<<showtext-4>>=
## Loading Google fonts (http://www.google.com/fonts)
font.add.google("Courgette", "courgette")
...
ggplot(my.data, aes(x, y, label = label)) +
  geom_text(hjust=1.5, family = "Lucida.Bright") +
  geom_point() +
  theme_grey(base_family="Lucida.Bright")
@
