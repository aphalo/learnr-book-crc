...
Code examples make use of some freely available \Rlang extension packages, which can be installed from \CRAN. One of them, \pkgname{learnrbook}, also available through \CRAN, contains data sets and files specific to this book. The \pkgname{learnrbook} package contains installation instructions and saved lists of the names of all other packages used in the book. The package also contains one script file per chapter with the \Rlang code for all the examples and exercises. Instructions on installing \Rpgrm, \pgrmname{Git}, \RStudio, compilers and other tools are available online. In many cases the IT staff at your employer or school will know how to install them, or they may even be included in the default computer setup. In addition, a web site supporting the book is available at \url{http://www.learnr-book.info} containing free supplemental chapters and errata. I do not provide detailed installation instructions here because they are likely to become outdated soon, while I can update package \pkgname{learnrbook} and the book's web site as needed.
...
To\index{R@{\Rlang}!help} access help pages through the command prompt we use function \Rfunction{help()} or a question mark. Every object exported by an \Rlang package (functions, methods, classes, data) is documented. Sometimes a single help page documents several \Rlang objects. Usually at the end of the help pages, some examples are given, which tend to help much in learning how to use the functions described. How to access \Rpgrm help is described in section \ref{sec:intro:using:R} on \pageref{sec:intro:using:R}.
